# Custom functions used in merge-data.R
# Original script author: Liam Shaw, liam.philip.shaw@gmail.com
...
# Function to convert JSON files to data frame
...
deduplicateAdmission <- function(admission.data.frame){
  ...
  admission.data.frame <- 
    admission.data.frame[!duplicated(admission.data.frame[,
                                                          seq(2, ncol(admission.data.frame))]),]
...
# SRN 21.01.20 - function to extract Admission.Diagnoses
## Definitely a more elegant way to do this, but works for now!

extractAdmissionDiagnoses<- function(dataFrame, anMonth){
  df<- subset(dataFrame, grepl(anMonth, dataFrame$Admission.DateTimeAdmission))
  
  # First, combine Mac and Macro - this should be incorporated into merge-data.R
  df <- df %>% mutate(Admission.Diagnoses = replace(
    Admission.Diagnoses, Admission.Diagnoses == "Macro", "Mac"))
  
  # Then, rename to avoid finding duplicate patterns
  df$Admission.Diagnoses<- gsub("BBA", "birthbefore", df$Admission.Diagnoses)
  df$Admission.Diagnoses<- gsub("BA", "HIE", df$Admission.Diagnoses)
  df$Admission.Diagnoses<- gsub("HIVXL", "HIVlow", df$Admission.Diagnoses)
  df$Admission.Diagnoses<- gsub("HIVX", "HIVhigh", df$Admission.Diagnoses)
  df$Admission.Diagnoses<- gsub("PREMRDS", "prrds", df$Admission.Diagnoses)
  
  # Now, keep track of counts for each diagnosis 
  AN<- sum(grepl("AN", df$Admission.Diagnoses, ignore.case = FALSE))
  BBA<- sum(grepl("birthbefore", df$Admission.Diagnoses, ignore.case = FALSE))
  BI<- sum(grepl("BI", df$Admission.Diagnoses, ignore.case = FALSE))
  Cong<- sum(grepl("Cong", df$Admission.Diagnoses, ignore.case = FALSE))
  CHD<- sum(grepl("CHD", df$Admission.Diagnoses, ignore.case = FALSE))
  DEHY<- sum(grepl("DEHY", df$Admission.Diagnoses, ignore.case = FALSE))
  DUM<- sum(grepl("DUM", df$Admission.Diagnoses, ignore.case = FALSE))
  FD<- sum(grepl("FD", df$Admission.Diagnoses, ignore.case = FALSE))
  HIVX<- sum(grepl("HIVhigh", df$Admission.Diagnoses, ignore.case = FALSE))
  HIVXL<- sum(grepl("HIVlow", df$Admission.Diagnoses, ignore.case = FALSE))
  BA<- sum(grepl("HIE", df$Admission.Diagnoses, ignore.case = FALSE))
  GSch<- sum(grepl("GSch", df$Admission.Diagnoses, ignore.case = FALSE))
  JA<- sum(grepl("JA", df$Admission.Diagnoses, ignore.case = FALSE))
  LBW<- sum(grepl("LBW", df$Admission.Diagnoses, ignore.case = FALSE))
  Mac<- sum(grepl("Mac", df$Admission.Diagnoses, ignore.case = FALSE))
  MA<- sum(grepl("MA", df$Admission.Diagnoses, ignore.case = FALSE))
  MecEx<- sum(grepl("MecEx", df$Admission.Diagnoses, ignore.case = FALSE))
  SEPS<- sum(grepl("SEPS", df$Admission.Diagnoses, ignore.case = FALSE))
  NB<- sum(grepl("NB", df$Admission.Diagnoses, ignore.case = FALSE))
  PN<- sum(grepl("PN", df$Admission.Diagnoses, ignore.case = FALSE))
  PREM<- sum(grepl("PREM", df$Admission.Diagnoses,  ignore.case = FALSE))
  PREMRDS<- sum(grepl("prrds", df$Admission.Diagnoses, ignore.case = FALSE))
  Risk<- sum(grepl("Risk", df$Admission.Diagnoses, ignore.case = FALSE))
  Safe<- sum(grepl("Safe", df$Admission.Diagnoses, ignore.case = FALSE))
  Twin<- sum(grepl("Twin", df$Admission.Diagnoses, ignore.case = FALSE))
  TTN<- sum(grepl("TTN", df$Admission.Diagnoses, ignore.case = FALSE))
  OTH<- sum(grepl("OTH", df$Admission.Diagnoses, ignore.case = FALSE))
  
  #now create dataframe of diagnosis vs. count
  df<- data.frame(diagnosis=c("AN", "BBA", "BI", "Cong", "CHD", "DEHY", "DUM", "FD",
                              "HIVX", "HIVXL", "BA", "GSch", "JA", "LBW", "Mac", "MA",
                              "MecEx", "SEPS", "NB", "PN", "PREM", "PREMRDS", "Risk",
                              "Safe", "Twin", "TTN", "OTH"),
                  count=c(AN, BBA, BI, Cong, CHD, DEHY, DUM, FD, HIVX, HIVXL,
                          BA, GSch, JA, LBW, Mac, MA, MecEx, SEPS, NB, PN, PREM,
                          PREMRDS, Risk, Safe, Twin, TTN, OTH))
