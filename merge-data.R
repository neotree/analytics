...
########## IMPORTANT ##########
##############################
# SET THESE THREE FILES TO ###
# THE PREVIOUS OUTPUT OF   ###
# THE LAST TIME THE SCRIPT ###
# WAS RUN (MOST RECENT).   ##
# 1. List of previously read JSON files
old.json.files <- read.csv('2019-11-12-LCSBQ3625H/LCSBQ3625H-2019-11-12-json-files.txt',
                           stringsAsFactors = F,
                           header = F)$V1
# 2. Old admission file (rds)
old.admission.df <- readRDS('2019-11-12-LCSBQ3625H/LCSBQ3625H-2019-11-12-admissions.rds')
# 3. Old discharge file (rds)
old.discharge.df <- readRDS('2019-11-12-LCSBQ3625H/LCSBQ3625H-2019-11-12-discharges.rds')
# Optional: set this depending on which computer you are
...
cat(c(admission.files, discharge.files, old.json.files),
    sep =  '\n',
    file = paste0(output.directory, '/',
                  run.string, '-', Sys.Date(), '-json-files.txt'))
